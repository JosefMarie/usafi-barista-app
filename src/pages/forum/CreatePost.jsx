        ...
        <div className="bg-[#F5DEB3] dark:bg-[#1c1916] min-h-screen flex flex-col font-display relative overflow-hidden">
            ...
            <header className="sticky top-0 z-50 bg-white/40 dark:bg-black/40 backdrop-blur-xl border-b border-white/20 dark:border-white/5 shadow-sm">
                <div className="flex items-center justify-between px-6 py-4 h-18">
                    ...
                    <h1 className="text-espresso dark:text-white text-lg font-serif font-black uppercase tracking-widest">{t('forum.create_title')}</h1>
                    <div className="w-10"></div> {/* Spacer for balance */}
                </div>
            </header>

            <main className="flex-1 flex flex-col w-full max-w-4xl mx-auto relative z-10 p-6 md:p-8">

                <div className="bg-white/40 dark:bg-black/40 backdrop-blur-xl rounded-[2.5rem] shadow-2xl border border-white/20 dark:border-white/5 p-8 flex-1 flex flex-col">
                    {/* Post Title Input */}
                    <div className="mb-8">
                        <label className="block mb-3 text-[10px] font-black text-espresso/40 dark:text-white/40 uppercase tracking-[0.2em] px-2">{t('forum.title_label')}</label>
                        <input
                            value={formData.title}
                            onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                            className="w-full bg-transparent border-b-2 border-espresso/10 dark:border-white/10 focus:border-espresso dark:focus:border-white text-3xl md:text-4xl font-serif font-bold text-espresso dark:text-white placeholder:text-espresso/20 dark:placeholder:text-white/20 px-2 py-4 focus:ring-0 transition-colors rounded-t-sm outline-none"
                    ...
                    <div className="mb-8">
                        <div className="px-2 flex items-center gap-2 mb-4">
                            <span className="material-symbols-outlined text-espresso/60 text-lg">label</span>
                            <h2 className="text-sm font-black text-espresso/60 dark:text-white/60 uppercase tracking-widest">{t('forum.select_topic')}</h2>
                        </div>
                        <div className="flex gap-3 overflow-x-auto hide-scrollbar pb-2 px-1">
                            {topics.map(topic => (
                                <button
                                    ...
                                    className={`group flex h-10 shrink-0 items-center justify-center gap-x-2 rounded-xl pl-4 pr-4 transition-all active:scale-95 whitespace-nowrap border ${formData.topic === topic.id
                                ...
                                >
                                    <span className="text-xs font-bold uppercase tracking-wide leading-normal">{topic.label}</span>
                                    {formData.topic === topic.id && <span className="material-symbols-outlined text-[16px]">check</span>}
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="flex-1 flex flex-col min-h-[300px]">
                        <label className="block mb-3 text-[10px] font-black text-espresso/40 dark:text-white/40 uppercase tracking-[0.2em] px-2 flex items-center justify-between">
                        ...
                        <textarea
                            ...
                            className="flex-1 w-full resize-none bg-white/30 dark:bg-white/5 border border-espresso/10 dark:border-white/10 focus:border-espresso/30 rounded-2xl text-espresso dark:text-white placeholder:text-espresso/20 dark:placeholder:text-white/20 p-6 text-lg leading-relaxed focus:ring-4 focus:ring-espresso/5 transition-all shadow-inner outline-none font-medium"
            ...
            {/* Bottom Action Bar */}
            <div className="sticky bottom-0 z-50 bg-white/40 dark:bg-black/40 backdrop-blur-xl border-t border-white/20 dark:border-white/5 p-4 pb-8 shadow-[0_-4px_30px_rgba(0,0,0,0.05)]">
                <div className="max-w-4xl mx-auto flex items-center justify-between w-full gap-4">
                    ...
                    <div className="flex items-center gap-1 text-espresso/60 dark:text-white/60">
                        <button aria-label="Bold" className="p-2 hover:bg-espresso/10 rounded-lg transition-colors active:text-espresso">
                            <span className="material-symbols-outlined text-[20px]">format_bold</span>
                        </button>
                        <button aria-label="Italic" className="p-2 hover:bg-espresso/10 rounded-lg transition-colors active:text-espresso">
                    ...
                    <button
                        ...
                        className="group relative flex-1 max-w-xs bg-espresso hover:bg-espresso/90 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-black uppercase tracking-widest h-14 rounded-2xl flex items-center justify-center gap-2 shadow-xl shadow-espresso/20 hover:-translate-y-0.5 active:translate-y-0 active:shadow-sm transition-all overflow-hidden"
                    >
